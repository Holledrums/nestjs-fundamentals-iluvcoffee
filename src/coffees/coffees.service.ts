...
import { Event as Event } from 'src/events/entities/event.entity';
...
export class CoffeesService {
  constructor(
    ...
    console.log('config:', coffeesConfiguration);
  }
  ...
  async create(createCoffeeDto: CreateCoffeeDto): Promise<Coffee> {
    const flavors = await this.getFlavors(createCoffeeDto);
  ...
  async update(id: string, updateCoffeeDto: UpdateCoffeeDto): Promise<Coffee> {
    const flavors = await this.getFlavors(updateCoffeeDto);
  ...
  private async getFlavors(
    coffeeDto: UpdateCoffeeDto | CreateCoffeeDto,
  ): Promise<Flavor[]> {
    if (coffeeDto.flavors) {
      return Promise.all(
        coffeeDto.flavors.map((name) => this.preloadFlavorsByName(name)),
      );
    }
    return [];
  }
